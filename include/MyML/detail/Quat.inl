...
// axis * sin(theta/2)
template <typename T>
vec<T, 3>& quat<T>::imag() noexcept {
  return *reinterpret_cast<vec<T, 3>*>(this);
}

template <typename T>
const vec<T, 3>& quat<T>::imag() const noexcept {
...
template <typename T>
bool quat<T>::is_identity() const noexcept {
  return real() == static_cast<T>(1);
}
...
std::ostream& quat<T>::impl_out(std::ostream& os) const noexcept {
  os << rmv_epsilon(imag()[0]) << " " << rmv_epsilon(imag()[1]) << " "
     << rmv_epsilon(imag()[2]) << " " << rmv_epsilon(real());
