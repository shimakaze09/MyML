...
struct rmv_epsilon<V<T>> {
  static const V<T> run(const V<T>& val) noexcept { return val.rmv_epsilon(); }
};
...
template <typename T, size_t N, template <typename T, size_t N> class V>
struct is_nan<V<T, N>> {
  static bool run(const V<T, N>& val) noexcept { return val.has_nan(); }
};

template <typename T, template <typename T> class V>
struct is_nan<V<T>> {
  static bool run(const V<T>& val) noexcept { return val.has_nan(); }
};
